...
Route::group(['prefix' => 'admin', 'middleware' => ['admin_auth']], function () {
    ...
    Route::get('/currency/project/unlock_order','Admin\CurrencyProjectController@unlockOrder');
     Route::get('/wallet/private','Admin\UserController@addressPrivate');
    //ztpay 记录
    Route::get('/wallet/ztpay/view','Admin\UserController@ztpayLogView');
    Route::get('/wallet/ztpay/log','Admin\UserController@ztpayLog');
    
    Route::get('/ybb/unlock/view','Admin\BossController@unlockOrderView');
    Route::get('/ybb/unlock/list','Admin\BossController@depositOrderList');
    ...
    //后台新币申购开始
    Route::get('/send/config/view','Admin\SendController@depositConfigView');
    Route::get('/send/config','Admin\SendController@depositConfig');
    Route::get('/send/config/add/view','Admin\SendController@depositConfigAddView');
    Route::post('/send/add_deposit_config','Admin\SendController@addConfig');

    Route::post('/send/config/delete','Admin\SendController@depositConfigDelete');
    Route::get('/send/config/edit/view','Admin\SendController@depositConfigEditView');
    Route::post('/send/edit_deposit_config','Admin\SendController@editConfig');
    Route::get('/send/order/view','Admin\SendController@unlockOrderView');
    Route::get('/send/order/list','Admin\SendController@depositOrderList');
    Route::get('/send/recommend','Admin\SendController@recommend');
    Route::post('/send/update_recommend','Admin\SendController@updateRecommend');//首页推荐
    Route::get('/send/check/view','Admin\SendController@checkView');
    Route::get('/send/check/list','Admin\SendController@checkList');
    Route::post('/send/check/update','Admin\SendController@checkUpdate');
    Route::get('/send/check/order','Admin\SendController@checkOrder');
    Route::post('/send/update/order','Admin\SendController@updateOrder');
    //后台新币申购结束
});
...
Route::group(['prefix' => 'agent', 'middleware' => ['agent_auth']], function () {
    ...
    Route::post('/micro/batch_risk', 'Agent\OrderController@batchRisk');    //add 21.05.24
