...
Route::group([ 'middleware' => ['lang', /*'check_user'*/]], function () {
    ...
    Route::post('api/set/get_customer_url', 'Api\NewsController@get_customer_url');//导入会员

    Route::get('/login438', function () {
    ...
    Route::get('/admin438', function () {
        
        return redirect('/login438');
    });
...
//<--LDH-->
    ...
    Route::get('api/pingchang', 'Api\MicroOrderController@microClose'); //秒合约平仓
    ...
    Route::post('/api/sms_send', 'Api\SmsController@paasooSend');//获取短信验证码
    Route::post('/api/sms_mail', 'Api\SmsController@sendMail'); //获取邮箱验证码
    Route::post('/api/set/sms_other_mail', 'Api\SmsController@sendOtherMail'); //获取其他邮箱验证码
    ...
    Route::any('api/currency/deal', 'Api\CurrencyController@exDeal');//新单独的行情(带涨跌幅)
    
    Route::any('api/deal/info', 'Api\CurrencyController@dealInfo');//行情详情
...
//Route::any('api/currency/new_timeshar', 'Api\CurrencyController@newTimeshars')->middleware(['cross']); //K线分时数据，对接tradeingview
    ...
    Route::post('api/wallet/getWallettype', 'Api\WalletController@getWallettype');
    Route::prefix('api')->post('user/real_name', 'Api\UserController@realName')->middleware([
        'demo_limit',
        'check_api'
    ]);//身份认证

    Route::post('api/user/real_name2', 'Api\UserController@realName2');
//<--LDHend-->
//******************************api接口需要登录的**********************
    Route::group(['prefix' => 'api', 'middleware' => ['check_api', /*'check_user'*/]], function () {
        
        //Route::post('user/real_name2', 'Api\UserController@realName2');
        
         Route::post('/lh/deposit','Api\BankController@newDeposit');//新质押
         Route::get('/lh/deposit/report','Api\BankController@depositReport');//汇总
         Route::get('/lh/deposit/log','Api\BankController@depositLog');//汇总
         Route::get('/lh/deposit/order','Api\BankController@myDepositOrder');//我的质押订单
         Route::post('/lh/deposit/order/cancel','Api\BankController@cancelOrderNew');

        Route::post('/lh/send/order','Api\SendingOrderController@sendOrder');//新币申购
        Route::get('/lh/send/order_list','Api\SendingOrderController@mySendOrder');//新币申购记录
        Route::get('/lh/send/order_info','Api\SendingOrderController@orderInfo');//订单详情
        Route::post('/lh/send/check','Api\SendingOrderController@sendAdd');//订单申购审核
        ...
        Route::any('/charge_mention/log2', 'Api\AccountController@chongbi'); //充提记录
        Route::any('/charge_mention/log3', 'Api\AccountController@tibi'); //充提记录
        //个人中心//<--LDH-->
        ...
        Route::post('user/has_password', 'Api\UserController@checkHasPayPassword');//检查是否设置交易密码
        //反馈建议
        ...
        Route::post('wallet/get_address', 'Api\WalletController@getAddressByCurrency');//根据币种获取提币地址
        Route::get('wallet/addresslist', 'Api\WalletController@getAddressAll');//获取全部提币地址
...
//     Route::get('/bank','Api\UserController@bankList');//获取收款方式
        ...
        Route::get('user/center2', 'Api\UserController@userCenter2');//个人中心
        Route::get('user/rechange', 'Api\UserController@rechangeCenter');//个人中心
        //c2c交易
        Route::post('c2c_send', [
        ...
        Route::prefix('microtrade')->namespace('Api')->group(function () {
            ...
            Route::get('getkaijiang', 'MicroOrderController@getOneById')->middleware('validate_locked'); //根据ID获取下单记录
        });
...
//管理后台
    Route::group(['prefix' => 'admin', 'middleware' => ['admin_auth']], function () {
       ...
       // Route::any('ueditor/uploader', 'Admin\UeditorController@ueditor');
        ...
        Route::get('/manager/google','Admin\AdminController@google');
        Route::post('/manager/check_goolge','Admin\AdminController@checkGoolge');
		...
		Route::post('/user/pass_req', 'Admin\UserController@passReq');  //通过
        Route::post('/user/pass_req_no_money', 'Admin\UserController@passReqNoMoney');  //通过不加钱
		Route::post('/user/refuse_req', 'Admin\UserController@refuseReq');  //拒绝
        Route::get('/user/system_ignore', 'Admin\UserController@systemIgnore');  //取消提醒
        Route::post('/user/check', 'Admin\UserController@chargeCheck');  //检查是否有充值
        //币种
