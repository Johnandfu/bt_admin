...
//        $all_money = bc_mul($fact_price, $num, 5);
        ...
        try {
            ...
            if($lan == 'zh'){
                ...
                $xm_submit = 'submit';
                $xm_Leveraged = 'Leveraged trading, price ';
                $kcbzj = 'Deduction of margin';
                $kcbzjsb =  'Failed to deduct margin:';
                $ggjy = 'Leverage trading, deduction of handling fees';
                $ksns = 'Failed to deduct the handling fee:';
            }elseif($lan == 'jp'){
                $xm_submit = '参加する';
                $xm_Leveraged = 'レバレッジ取引、価格';
                $kcbzj = '証拠金の控除';
                $kcbzjsb =  '証拠金の控除に失敗しました:';
                $ggjy = 'レバレッジ取引、手数料の控除';
                $ksns = '手数料の控除に失敗しました:';
            }elseif($lan == 'kr'){
                 $xm_submit = '제출하다';
                $xm_Leveraged = '레버리지 거래, 가격';
                $kcbzj = '증거금 공제';
                $kcbzjsb =  '마진 차감 실패:';
                $ggjy = '레버리지 트레이딩, 수수료 차감';
                $ksns = '수수료 공제 실패:';
            }elseif($lan == 'hk'){
                 $xm_submit = '提交';
                $xm_Leveraged = '槓桿交易,價格';
                $kcbzj = '扣除保證金';
                $kcbzjsb =  '扣除保證金失敗:';
                $ggjy = '槓桿交易,扣除手續費';
                $ksns = '扣除手續費失敗:';
            ...
            $result = change_wallet_balance(
                ...
                -$caution_money,
                AccountLog::LEVER_TRANSACTION,
                $xm_submit . $currency_match->symbol . $xm_Leveraged . $fact_price . $kcbzj,
                false,
                0,
                0,
                serialize([
            ...
            if ($result !== true) {
                throw new \Exception($this->returnStr($kcbzjsb) . $result);
            }
            //扣除手续费
            $result = change_wallet_balance(
                $legal,
                ...
                -$trade_fee,
                AccountLog::LEVER_TRANSACTION_FEE,
                $xm_submit. $currency_match->symbol . $ggjy,
                false,
                0,
                0,
                serialize([
            ...
            if ($result !== true) {
                throw new \Exception($this->returnStr($ksns) . $result);
            }
            DB::commit();
