...
class UserController extends Controller
{
    ...
    public function saveCashInfo(Request $request)
    {
        ...
        $bank_address = $request->get('bank_address', '');//银行地址
        $swift_code = $request->get('swift_code', '');//swift代码
        $residence_address = $request->get('residence_address', '');//收款人地址
        $postal_code = $request->get('postal_code', '');//微信二维码
        ...
        if (!empty($swift_code)) {
            $cash_info->swift_code = $swift_code;
        }
        if (!empty($residence_address)) {
            $cash_info->residence_address = $residence_address;
        }
        if (!empty($postal_code)) {
            $cash_info->postal_code = $postal_code;
        }
...
//        var_dump($now_price);die;
    ...
    public function cashInfo()
    {
        ...
        $banks = Bank::where("lang",$lang)->select('id','name')->get();
        $result->banks = $banks;
    ...
    //检测是否设置了交易密码
    public function checkHasPayPassword(){
        $user_id = Users::getUserId();
        $user = Users::find($user_id);
    ...
    public function updatePayPassword()
    {
        ...
        if ($user->pay_password && Users::MakePassword($oldpassword) != $user->pay_password) { //如果有原密码 且密码不相等 则进行验证报错
    ...
    public function invite()
    {
        ...
        $data['return_rate1'] = Setting::getValueByKey('partner_ratio_lv1', 0);
        $data['return_rate2'] = Setting::getValueByKey('partner_ratio_lv2', 0);
        $data['return_rate3'] = Setting::getValueByKey('partner_ratio_lv3', 0);
        $data['total_return_rate'] = $data['return_rate1'] + $data['return_rate2'] + $data['return_rate3'];
    ...
    public function myInviteList()
    {
        ...
        $chat_list = $list->orderBy('id', 'desc')->select(['id','parent_id','account_number','phone','time','email'])->paginate($limit);

        //$datas = $chat_list->items()->select(['account','create_date','id','parent_id']);
        
        return $this->success($chat_list);
    ...
    public function myAccountReturn()
    {
        ...
        */
        ...
        $chat_list = $list->orderBy('id', 'desc')->select(['id','type','currency', 'user_id','created_time','value'])->paginate($limit);
        $datas = [];
        
        $tmp = $chat_list->toArray();
        foreach ($tmp['data'] as &$item){
            unset($item['type_info_m'],$item['type_info_e'],$item['type_info_j'],$item['before'],$item['after'],$item['account'],$item['account_number']);
            //$item['account_number'] = $this->accountFormat($item['account_number']);
            //$item['account'] = $this->accountFormat($item['account']); 
            if($item['from_user_id']){
                $from_user = Users::where('id',$item['from_user_id'])->first(['account_number']);
                $item['from_account_number'] = $this->accountFormat($from_user['account_number']);
            }else{
                $item['from_account_number'] = '-';
            }
        }
    ...
    public function accountFormat($str)
    {
        if (strpos($str, '@')) {
            $n = strripos($str, '@');
            $value = mb_substr($str, 0, 2) . '******' . mb_substr($str, $n);
        } else {
            $value = mb_substr($str, 0, 3) . '******' . mb_substr($str, -3, 3);
        }
        return $value;
    }
...
//        $user_id = Users::getUserId();
    ...
    public function info(Request $request)
    {
        ...
        $user = Users::where("id",$user_id)->first(['id','phone','email','head_portrait','status','pay_password','extension_code','account_number']);
        //$user = Users::where("id", $user_id)->first();
        if (empty($user)) {
            return $this->error("会员未找到");
        }
...
//        $legal_wallet['CNY'] = '';
    ...
    public function realName()
    {
        ...
        $country = Input::get("country", "");//真实姓名
...
//        var_dump($userreal_number);die;
        ...
        try {
            ...
            $userreal->name = $name;
            $userreal->country = $country;
            $userreal->type = 1;    //初级认证
    ...
    public function realName2(){
        ...
        if (empty($front_pic) || empty($reverse_pic) || empty($hand_pic) ) {
            return $this->error("请提交完整的信息");
        }  
        ...
        $userreal = UserReal::where('user_id', $user_id)->first();  //初级认证
        if (empty($userreal) || ($userreal->type=='1' && $userreal->review_status!='2') ) {
            return $this->error("您还没有通过实名认证");
        }
        
        if($userreal->type=='2'){//已申请高级认证
            return $this->error("您已经申请过了");
        }
        
        if($card_id != $userreal->card_id){
            return $this->error('与初级认证证件不一致');
        }
        ...
        try {
            ...
            $userreal->user_id = $user_id;
            $userreal->type = 2;    //设置为高级认证
            $userreal->review_status = 1;   //重置为待审核状态
            //$userreal->name = $name;
            //$userreal->card_id = $card_id;
            //$userreal->create_time = time();
            $userreal->front_pic = $front_pic;
    ...
    public function userCenter()
    {
        ...
        $user = Users::where("id", $user_id)->first(['id', 'phone', 'email','account_number']);
        if (empty($user)) {
            return $this->error("会员未找到");
        }
        $userreal = UserReal::where(['user_id'=> $user_id])->first(); //初级
        ...
        //$user['my_account'] = $user['phone'] ? $user['phone'] : $user['email']; 
    ...
    public function userCenter2()
    {
      ...
      //  $userreal = UserReal::where('user_id', $user_id)->first();
        $userreal = UserReal::where(['user_id'=>$user_id,'type'=>2])->first();
    ...
    public function e_pwd()
    {
        ...
        if($type=='1'){
            $pwd =  Users::MakePassword($old_password, $tra_user->type);
            if($pwd != $tra_user['password']){
                return $this->error('用户密码错误');
            }
