...
//            }
            ...
            if($lan == 'zh'){
               ...
               $pccl = 'Storm warehouse handling';
                $yeth = 'Balance (returned margin:';
                $jszyk = 'Settlement total profit and loss';
            }elseif($lan == 'jp'){
                $pccl = 'ストームウェアハウスの取り扱い';
                $yeth = '残高（返還証拠金：';
                $jszyk = '決済合計損益';
            }elseif($lan == 'kr'){
               $pccl = '폭풍우 창고 취급';
               ...
               $pccl = '暴倉處理';
                ...
                $zjpc = 'Liquidation funds processing';
                $pcmsg = 'Failed to close position: Failed to update the processing status';
            }
            ...
            $result = change_wallet_balance(
                ...
                $pccl. $legal_name . $yeth. $caution_money_total . $jszyk. $profits_total . ')',
                false,
                0,
                $diff == 0 ? 0 : 1, //1代表有差额
                $extra_data,
        ...
        try {
            ...
            if($lan == 'zh'){
               ...
               $pcmsg = 'Failed to close position: Failed to update the processing status';
                ...
                $zjpc = '清算資金処理';
                $pcmsg = 'ポジションを閉じることができませんでした：処理ステータスの更新に失敗しました';
            }elseif($lan == 'kr'){
               $zjpc = '청산 자금 처리';
               $pcmsg = '위치 닫기 실패: 처리 상태 업데이트 실패';
            }elseif($lan == 'hk'){
               $zjpc = '平倉資金處理';
               $pcmsg = '平倉失敗:更新處理狀態失敗';
            }else{
